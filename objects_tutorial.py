'''
 ...
 ans)    An object is a 'thing' that contains data and functions to manipulate 
         (perform operations on) that data. An object contains data and methods.
         ...
         So, here, we are defining objects x and y by saying x \in R^2, and y \in R^2.
         In OOP, as in all other types of programming I know of, we say R^2 x and R^2 y.
         That defines x and y as members of R^2, or in programming terms the type of x and y
         is R^2. 

         We also say that x and y are 'instances' of the 'class' R2. We also say we can 
         ...
         Clearly using addvector will quickly get out of hand. Yes, I know one could write a 
         function to accept an arbitrary number of arguments, but

         v4 = v1+v2+v3 is still cleaner and more elegant than v4=addvector(v1,v2,v3) 
         
         Note: v4=v1+v2+v3 needs a compiler/interpreter smart enough to understand that 
         the result of v1+v2 is a vector, create that temporary vector v1+v2 and then add
...
*******************************************************************************************************************
         ...
         If you really want to take the object (data and methods) that are labelled 'a' and make a copy, and have that 
...
# Defining an excessive number of functions will distract.
...
def vector_add(v1,v2):
    ...
    this code will work. As you can see, we don't need different functions to add different data types
...
# Other langauges like C++ don't let you do so. Let's look at the variables we defined.
...
# The special property of the __init__ method is that it is, for all practical purposes,
# the first method to be called after the class is instantiated. The method __init__, in our case, takes 3 arguments
# 'self' is the instance of an object. 'x' and 'y' are user supplied arguments.
# We saw earlier that 'v1=Vector()' was enough to instantiate
# the class, allocate memory for it and create an object. You can think of 'self' as this object,
...
# The Python interpreter does v1+v2, stores the result in a temporary vector, and then adds v3 to that temporary vector
