...
% Average error, histogram of the scaled error, histogram of total dataset
...
\eer
The above equations imply that $u_y$ and $\epsilon_{yy}$ can be regarded as independent variables. Therefore in this work, we restrict our attention to only single channel images which represent either $u_y$ or $\epsilon_{yy}$.
\end{subequations}
...
\begin{subequations}
  ...
  \eer
\end{subequations}
In the above equations, $\mu_{j}^{i,true}$ is the $j^{th}$ component of vector $\mu^{i,true}$ containing true values of the shear modulus for the $i^{th}$ test example, \textit{restricted to the inclusion(s)} only. $\mu_{j}^{i,predicted}$ is the $j^{th}$ component of a vector $\mu^{i,predicted}$ containing corresponding predictions. $n_{inc_i}$ is the number of nodes in the background for the $i^{th}$ test examle. $n_{test}$ is the number of test examples, in our case $800$. The quantities $\eta_i,\eta_{ave}$ capture how accurately the shear modulus of the inclusions is predicted and, depending on its sign, can tell us whether it is overpredicted or underpredicted. Negative sign indicates underprediction and positive sign indicates overprediction.

The \textit{background scaled error} and its average can be defined similarly, by replacing $n_{inc_i}$ with $n_{back_i}$ and restricting the vectors $\mu^{i,true}$ and $\mu^{i,predicted}$ to the background only.
...
Figures (\ref{fig:cnn1resultb}-\ref{fig:cnn1resultd}) show that the regions of low shear modulus are not accurately reconstructed. Their reconstruction can be best described as 'wisps'. Figure (\ref{fig:cnn1histo}) shows histograms in which the scaled error, equation (\ref{eqn:scalederror}), is on the x-axis and the y-axis represents the fraction of examples in each bin (number of examples in each bin divided by the total number of examples). It is seen that the examples appear to be normally distributed around a mean of ${0.207}$ and that $80\%$ test examples have scaled error of less than $0.25$. The maximum and minimum shear moduli in the prediction are $5.32$ and $0.631$. 
% Losses for CNN1
\begin{figure}[!h]
  \centering
...
\begin{figure}[!h]
  ...
  \begin{subfigure}[c]{0.48\linewidth}
    \includegraphics[totalheight=5.0cm]{Figures/Results1/ex1/mu.png}
    \subcaption{\label{fig:cnn1resulta}Best (0.0786)}
  \end{subfigure}
  %
  \begin{subfigure}[c]{0.48\linewidth}
    \includegraphics[totalheight=5.0cm]{Figures/Results1/ex2/mu.png}
    \subcaption{\label{fig:cnn1resultb}Worst (0.391)}
  \end{subfigure}
  %
  \begin{subfigure}[c]{0.48\linewidth}
    \includegraphics[totalheight=5.0cm]{Figures/Results1/ex3/mu.png}
    \subcaption{\label{fig:cnn1resultc}(0.286)}
  \end{subfigure}
  %
  \begin{subfigure}[c]{0.48\linewidth}
    \includegraphics[totalheight=5.0cm]{Figures/Results1/ex4/mu.png}
    \subcaption{\label{fig:cnn1resultd}(0.252)}
  \end{subfigure}
\caption{\label{fig:cnn1result} Results for CNN1. The numbers in the brackets are the scaled errors as defined in equation (\ref{eqn:scalederror}).}  
\end{figure}
%
% Histograms for CNN1
\begin{figure}[!h]
\captionsetup[subfigure]{justification=centering}
  \centering
  \begin{subfigure}[c]{0.45\linewidth}
    \includegraphics[totalheight=5.0cm]{Figures/Results1/histogram.png}
    \subcaption{Histogram}
  \end{subfigure}
%  
  \begin{subfigure}[c]{0.45\linewidth}
    \includegraphics[totalheight=5.0cm]{Figures/Results1/cumulative.png}
...
\begin{figure}[!h]
  \centering
  \begin{subfigure}[c]{0.48\linewidth}
    \includegraphics[totalheight=5cm]{Figures/Results2/ex1/mu.png}
    \subcaption{\label{fig:cnn2resulta}Best (0.0972)}
  \end{subfigure}
  %
  \begin{subfigure}[c]{0.48\linewidth}
    \includegraphics[totalheight=5cm]{Figures/Results2/ex2/mu.png}
    \subcaption{\label{fig:cnn2resultb}Worst (0.494)}
  \end{subfigure}
  %
  \begin{subfigure}[c]{0.48\linewidth}
    \includegraphics[totalheight=5cm]{Figures/Results2/ex3/mu.png}
    \subcaption{\label{fig:cnn2resultc}(0.308)}
  \end{subfigure}
  %
  \begin{subfigure}[c]{0.48\linewidth}
    \includegraphics[totalheight=5cm]{Figures/Results2/ex4/mu.png}
    \subcaption{\label{fig:cnn2resultd}(0.279)}
  \end{subfigure}
\caption{\label{fig:cnn2result} Results for CNN2. The numbers in the brackets are the scaled errors as defined in equation (\ref{eqn:scalederror}). Figures (c) and (d) are the same test examples presented in figure (\ref{fig:cnn1result}).}  
\end{figure}
%
% Histograms for CNN2
\begin{figure}[!h]
\captionsetup[subfigure]{justification=centering}
  \centering
  \begin{subfigure}[c]{0.45\linewidth}
    \includegraphics[totalheight=5.0cm]{Figures/Results2/histogram.png}
    \subcaption{Histogram}
  \end{subfigure}
%  
  \begin{subfigure}[c]{0.45\linewidth}
    \includegraphics[totalheight=5.0cm]{Figures/Results2/cumulative.png}
    \subcaption{Cumulative histogram}
  \end{subfigure}
  %
...
In this section, we evaluate the ability of the CNN to generalize to the type of examples outside its training set. As noted in table (\ref{table:cnnparams}) we train the CNN using data which was generated using only one inclusion. But we evaluate it on data containing one, two or three inclusions. The training and validation losses for CNN3 are shown in figure (\ref{fig:cnn3losses}). For this network, the average scaled error defined in equation (\ref{eqn:averagescalederror}) is $0.229$ which is only slightly worse than that of CNN1 and the same as that of CNN2 when rounded to three significant digits, which we consider surprising. The standard deviation of the average scaled error is $0.0727$ which is significantly higher than CNN1 and CNN2. The average inclusion scaled error is $-0.187$ and the average background scaled error is $-0.0367$. These indicate that, on average, the shear modulus of the inclusions is underpredicted, and that of the background is predicted accurately. The underprediction of the inclusion stiffness is more severe than the results for CNN1 and CNN2.
...
The maximum and minimum shear moduli in the prediction are $5.37$ and $0.513$.
...
\begin{figure}[!h]
  \centering
  \begin{subfigure}[c]{0.48\linewidth}
    \includegraphics[totalheight=5cm]{Figures/Results3/ex1/mu.png}
    \subcaption{\label{fig:cnn3resulta}Best (0.0769)}
  \end{subfigure}
  %
  \begin{subfigure}[c]{0.48\linewidth}
    \includegraphics[totalheight=5cm]{Figures/Results3/ex2/mu.png}
    \subcaption{\label{fig:cnn3resultb}Worst (0.497)}
  \end{subfigure}
  %
  \begin{subfigure}[c]{0.48\linewidth}
    \includegraphics[totalheight=5cm]{Figures/Results3/ex3/mu.png}
    \subcaption{\label{fig:cnn3resultc}(0.397)}
  \end{subfigure}
  %
  \begin{subfigure}[c]{0.48\linewidth}
    \includegraphics[totalheight=5cm]{Figures/Results3/ex4/mu.png}
    \subcaption{\label{fig:cnn3resultd}(0.303)}
  \end{subfigure}
\caption{\label{fig:cnn3result} Results for CNN3. The numbers in the brackets are the scaled errors as defined in equation (\ref{eqn:scalederror}). Figures (c) and (d) are the same test examples presented in figures (\ref{fig:cnn1result}) and (\ref{fig:cnn2result}).}  
\end{figure}
%
% Histograms for CNN3
\begin{figure}[!h]
\captionsetup[subfigure]{justification=centering}
  \centering
  \begin{subfigure}[c]{0.45\linewidth}
    \includegraphics[totalheight=5.0cm]{Figures/Results3/histogram.png}
    \subcaption{Histogram}
  \end{subfigure}
%  
  \begin{subfigure}[c]{0.45\linewidth}
    \includegraphics[totalheight=5.0cm]{Figures/Results3/cumulative.png}
    \subcaption{Cumulative histogram}
  \end{subfigure}
  %
...
\item{We think that Fourier analysis of these filters, as reported in \cite{paper:pateloberai2019} would be valuable. After understanding the physical meaning of each filter, better filters could perhaps be constructed manually.}
