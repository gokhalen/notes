...
We conclude by commenting that enforcing constraints on the shear modulus is easy in traditional iterative methods because we can just supply bound constraints to the optimization algorithm. On the other hand, for neural networks, constrainng the shear modulus requires constraining the range of the output layer activatio which causes the gradients of the activation function to be zero, as the input goes further away from zero.
...
\item{We think that using millions of training examples will improve the performance of the CNNs evaluated in this work. Using a large number of examples will also help incorporate large ranges of $\mu$, complex shapes for the inclusions, many different boundary conditions, and training with many noisy displacement or strain fields generated from noiseless fields. These noisy fields, corresponding to different realizations of noise, will correspond to the same true shear modulus field. Highly optimized finite element solvers and supporting state of the art computer hardware such as high end clusters of CPUs and GPUs and fast storage such as SSDs will be required for data generation and CNN training.}
\end{enumerate}
%\clearpage
\begin{appendices}
