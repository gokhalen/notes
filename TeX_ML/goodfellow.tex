...
\url{https://www.cs.ubc.ca/~murphyk/Teaching/CS540-Fall08/L13.pdf}\\
Quadratic approximation of the regularized objective function
\beq
\tilde{J}(\pmb{w}) = J(\pmb{w^*}) + \sum_{i}\Big{[}\frac{1}{2}H_{i,i}(w_i-w^*_i) + \alpha|w_i|\Big{]} \qquad H_{i,i}>0, \alpha > 0
\eeq
Equations are derived through black-magic of subdifferentials/subgradients. According to reference 2 on page 8, for the $j^{th}$ component of the minimizer of the above equation if $|w_j|>0$, the optimality conditions are
\ber
H_{j,j}(w_j-w^*_j) + \alpha\sign(w_j) = 0 \text{ if } |w_j| > 0 \text{ no sum on } j\\
\label{eqn:subdiffmaster}
w_j = w_j^* - \alpha\frac{\sign(w_j)}{H_{j,j}} \text{ no sum on } j
\eer
I am not able to derive 7.23 from the above equation.
%We consider two cases 1) $w_j>0$  and, 2)$w_j<0$. In the first case, we will derive an equation and set $w_j$ to zero if the equation drives $w_j$ below zero. In the second case, we will set $w_j$ to zero if the equation drives $w_j$ above zero.\\
%textbf{Case 1:} $w_j>0$
%Equation (\ref{eqn:subdiffmaster}) becomes,
%\beq
%w_j =  w_j^* - \frac{\alpha}{H_{j,j}} \,\, \text{...because }\sign{w_j} = 1 \text{ bcos } w_j>0
%\eeq
%Note $\alpha>0,H_{j,j}>0$.\\
%If $w_j^*>0$ this equation can be written as
%\beq
%w_j =  \sign({w_j^*})|w_j^*| - \sign({w_j^*})\frac{\alpha}{H_{j,j}} \text{...}\sign({w_j})=1\\ 
%\eeq
%We add a $\max$ term to make $w_j$ zero when the RHS forces $w_j$ to be less than zero. We get
%\beq
%w_j = \sign({w_j^*})\max\Big{\{}|w_j^*| - \frac{\alpha}{H_{j,j}},0\Big{\}}
%\eeq
%
%\textbf{Case 2:} $w_j<0$
%Equation (\ref{eqn:subdiffmaster}) becomes,
