...
Given a decoder $\pmb{D}$, such that $\pmb{D}^T\pmb{D}=1$ what should the encoder be? The idea is that of all the points $\pmb{c}$ that one could map $\pmb{x}$ to, we should choose the point $\pmb{c}^*$ that minimizes $\|\pmb{x}-g(\pmb{c})\|_2$. Taking derivatives we find that the minimizer satisfies $\pmb{c}^* = \pmb{D}^T\pmb{x}$. So, $\pmb{D}^T$ is our encoder. The book has a more detailed derivation. 
...
\end{table}
\section{Softmax}
\subsection{Equation 6.3.1}
The following tells us the fraction of examples belonging to class $i$ in the neighborhoold of $\pmb{x}^{k}$.
\url{https://stats.stackexchange.com/questions/553522/softmax-equation-from-goodfellows-deep-learning-book}
\beq
\text{softmax}(\pmb{z}(\pmb{x}^{(k)},\pmb{\theta}))_i = \frac{\sum_{j=1}^{m}\pmb{1}_{y^{(j)}=i,\pmb{x}^{(j)}=\pmb{x}}}{\sum_{j=1}^{m}\pmb{1}_{x^{(j)}=\pmb{x}}}
\eeq
...
\subsection{From IISc course}
Dropout lets the NN learn which neurons are important.
%
%
%
