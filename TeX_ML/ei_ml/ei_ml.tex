...
We believe that solving the inverse problem with CNNs can combine the best characteristics of the direct and iterative approaches. The CNN based approach can yield a quick answer (once time has been spent up front to train the CNN), can accommodate complex constitutive relations, can work with incomplete data (e.g. only a single component of a displacement field) and can work with noisy data. On the other hand, if data which is unlike what the CNN has been trained on is seen, then it is doubtful that the CNN will be able to make correct predictions.
...
\begin{figure}[h]
  ...
  \caption{\label{fig:oneinc:7}Imaging one inclusion (softplus): result 7. Shear modulus of this inclusion is slightly overpredicted. There are areas of low shear modulus adjacent to the inclusion especially in figures (b),(c) and (f)-(i).}
\end{figure}
\newpage
\clearpage
% Figure 7 (onetanh)
...
\begin{figure}[h]
  ...
  \caption{\label{fig:oneinc:8}Imaging one inclusion (softplus): result 8. Shear modulus of this small inclusion is severely underpredicted in all figures. There are regions of low shear modulus adjacent to the inclusion in figures (b),(c),(h) and (i).}
\end{figure}
\newpage
\clearpage
% Figure 8 (onetanh)
\begin{figure}[h]
  ...
  \caption{\label{fig:oneinctanh:8}Imaging one inclusion (twisted tanh): result 8. Unlike the previous result with softplus, the inclusion is clearly visible in all figures. Result (b) is good and the results get progressively worse towards result (i).}
\end{figure}
\newpage
\clearpage
% Figure 9 (softplus)
...
\begin{figure}[h]
  ...
  \caption{\label{fig:oneinctanh:14}Imaging one inclusion (twisted tanh): result 14. The inclusions are not clearly defined. They are fuzzy. This may be because the true shear modulus is low.}
\end{figure}
\newpage
\clearpage
% Figure 15 (softplus)
\begin{figure}[h]
  ...
  \caption{\label{fig:oneinc:15}Imaging one inclusion (softplus): result 15. Medium sized inclusions are detected fairly well. There is slight overprediction of the shear modulus. There are regions of low shear modulus adjacent to the inclusions.}
\end{figure}
\newpage
\clearpage
% Figure 15 (onetanh)
...
\begin{figure}[h]
  ...
  \caption{\label{fig:threeinctanh:7}Imaging up to 3 inclusions (twisted tanh): result 7. The overlapping inclusions are combined into a single inclusion whose shear modulus is slightly overpredicted. There are no regions of low shear modulus adjacent to the inclusion. }
\end{figure}
\newpage
\clearpage
% Figure 8 3inc
\begin{figure}[h]
  ...
  \caption{\label{fig:threeinc:8}Imaging up to 3 inclusions (softplus): result 8. The shear modulus of both inclusions is overpredicted. There are regions of low shear modulus adjacent to the inclusion, especially in (f),(h) and (i).}
\end{figure}
\newpage
\clearpage
% Figure 8 (threetanh)
