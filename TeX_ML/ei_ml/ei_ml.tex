...
\end{enumerate}
The problem with the above algorithm is that the storage required in step (3) is beyond enormous. There are ${n_{\mu}}^{n_{nodes}}$ possible shear modulus fields. This is an enormous number. We need an algorithm to search that space efficiently. Hence, we need neural networks.
%
\begin{figure}[h]
   \centering
