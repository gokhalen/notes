...
See section (\ref{sect:symmetric}). I think the constraint $\pmb{w}^T\pmb{x}^{(j)}+b=1$ needs to be enforced for a PAIR of closest points from the positive and negative class. (somehow, and intelligently). If it is enforced for only one point, it ends up maximizing the distance from that point only. (not sure about this) (this thinking is wrong)  in svm\_two\_point.py We can see that setting $\pmb{w}^T\pmb{x}^{(j)}+b=1$ for only one point suffices to find the separating hyperplane. I'm 
